...
if __name__ == "__main__":
  ...
  try:
    ...
    try:
      ...
      parser.add_argument("-t", "--usertype", type=int, default=2, help = "User type 1= student, 2 = parent")
  ...
  if weekday < 5:
    output = dict(
      ...
      meals = api.fetch_lunch_menu(student=settings[0][0])
      )
    ...
    if len(settings) < 1:
      ...
      for i in range(len(settings)+1):
        output.update(
          ...
          'info_'+ str(i) :api.fetch_info(student=settings[0][i]),
          'preschool_'+ str(i) :api.fetch_preschool_schedule(student=settings[0][i]),
          'schedule_'+ str(i) :api.fetch_schedule(student=settings[0][i])}
          )

    if len(output["meals"]) >= weekday and len(output["meals"]) != 0:
    	output["meal"] = output["meals"][weekday]
  else:
    output = dict(
      updated = int(datetime.timestamp(datetime.now())),
      icon = "mdi:school",
